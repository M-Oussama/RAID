        ...
        table.DataTable({
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        return '\
                        \<a href="dash/users/' + row.id + '/pdf" class="btn btn-sm btn-clean btn-icon" title="PDF">\
                            <i class="far fa-file-pdf">\
                            </i>\
                        </a>\
                        @canany(['edit-user','delete-user'])
                            @can('edit-user')
                            <a href="dash/security/assistance/' + row.id + '/edit" class="btn btn-sm btn-clean btn-icon mr-2" title="Edit details">\
    ...
    <div class="container">
        ...
        <div class="card card-custom gutter-b">
            <div class="card-header flex-wrap py-3">
                <div class="card-title">
                    <h3 class="card-label">لائحة الموظفين
                        @if(count($employees) > 0)
                                <span class="d-block text-muted pt-2 font-size-sm"> موظف{{count($employees)}}  </span>
                        @else
                            <span class="d-block text-muted pt-2 font-size-sm">اللائحة فارغة </span>
                        @endif
                    </h3>
                </div>
                <div class="card-toolbar">
                                    ...
                                    @can('list-user')
                                        <li class="navi-item">
                                            <a href="dash/security/assistance/export" class="navi-link">
                                                ...
                                                <span class="navi-text">استخراج لائحة الموظفين</span>
                                            </a>
                                        </li>
                                    @endcan
                                </ul>
                    ...
                    <!--begin::Button-->
                    @can('create-user')
                        <a href="dash/security/assistance/create" class="btn btn-primary font-weight-bolder">
                            <span class="svg-icon svg-icon-md">
            ...
            <div class="card-body">
                <form id="deleteMultiForm" action="dash/security/assistance/delete-multi" method="post">
                @csrf
                <!--begin: Datatable-->
                ...
                <form id="deleteForm" action="dash/security/assistance/{id}" method="post">
                    ...
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalFormTitle">هل أنت متأكد من حذف الموظف؟</h5>
                        </div>
        ...
        <div class="modal fade" id="deleteMultiModal" tabindex="-1" aria-labelledby="exampleModalFormTitle"
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalFormTitle">هل أنت متأكد من حذف الموظفين؟</h5>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-light-primary font-weight-bold" data-dismiss="modal">
                            إغلاق
                        </button>
                        <button id="deleteMultiSubmit" type="submit" class="btn btn-danger font-weight-bold">حذف
                        </button>
                    </div>
                </div>
            </div>
